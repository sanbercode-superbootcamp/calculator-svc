...
describe('Calculator Substraction', function () {
    ...
    it('should do number substraction', async function () {
        let result = await get("http://localhost:3000/substract?i1=10&i2=2", { json: true });
    ...
    it('should return errorcode 400 if not a number', async function () {
        let error = null;
        try {
            let result = await get("http://localhost:3000/substract?i10=2&i2=haha", { json: true });
        } catch (err) {
            error = err;
        }
